...
/* ---------- Draft: card pool for packs ---------- */
const PVP_PACK_POOL = ['heavy','slash','ironwall','potion','quickdraw','fireball','strike','guard'];
...
function createBattle(seed, floor, customDeck){
  ...
  const baseDeck = (customDeck && customDeck.length) ? customDeck.slice() : pvpStarterDeck();
...
function B_playCard(B, handIdx){
  ...
  if(card.dmg){
    ...
    const bonus = (card.type==='attack' ? (B.atkBonus||0) : 0);
    for(let h=0; h<hits; h++) B_dealToEnemy(B, card.dmg + bonus, log);
  }
